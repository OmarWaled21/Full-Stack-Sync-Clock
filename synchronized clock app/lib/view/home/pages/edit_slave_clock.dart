...
class EditClock extends StatefulWidget {
  final SlaveClockModel clock;
  const EditClock({super.key, required this.clock});

  @override
  State<EditClock> createState() => _EditClockState();
}

class _EditClockState extends State<EditClock> {
  ...
  void initState() {
    ...
    _nameController = TextEditingController(text: widget.clock.name);
    _maxTempController = TextEditingController(text: widget.clock.maxTempThreshold.toString());
    _minTempController = TextEditingController(text: widget.clock.minTempThreshold.toString());
  }
              ...
              child: CustomButton(
                onPressed: () {
                  context.read<HomeCubit>().editSlaveClock(
                    deviceId: widget.clock.deviceId,
